...
class MatBarlatYld2000Parser(Parser):
    def __init__(self):
        super().__init__()
    ...
    def _12p0(self, line_raw):
        ...
        if self.line_number == 5:
            ...
            pr_raw = self.temp[3]
            fit_raw = self.temp[4]
            beta_raw = self.temp[5]
            iter_raw = self.temp[6]
            iscale_raw = self.temp[7]
            k_raw = self.temp[8]
            e0_raw = self.temp[9]
            n_raw = self.temp[10]
            c_raw = self.temp[11]
            p_raw = self.temp[12]
            hard_raw = self.temp[13]
            a_raw = self.temp[14]
            alpha1_raw = self.temp[15]
            alpha2_raw = self.temp[16]
            alpha3_raw = self.temp[17]
            alpha4_raw = self.temp[18]
            alpha5_raw = self.temp[19]
            alpha6_raw = self.temp[20]
            alpha7_raw = self.temp[21]
            alpha8_raw = self.temp[22]
            aopt_raw = self.temp[23]
            offang_raw = self.temp[24]
            p4_raw = self.temp[25]
            htflag_raw = self.temp[26]
            hta_raw = self.temp[27]
            htb_raw = self.temp[28]
            htc_raw = self.temp[29]
            htd_raw = self.temp[30]
            a1_raw = self.temp[31]
            a2_raw = self.temp[32]
            a3_raw = self.temp[33]
