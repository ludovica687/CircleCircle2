...
class MatPiecewiseLinearPlasticityLogInterpolationParser(Parser):
    def __init__(self):
        super().__init__()
    ...
    def _12p0(self, line_raw):
        ...
        if self.line_number == 3:
            ...
            vp_raw = self.temp[12]
            lcf_raw = self.temp[13]

            eps1_raw = self.temp[14]
            eps2_raw = self.temp[15]
            eps3_raw = self.temp[16]
            eps4_raw = self.temp[17]
            eps5_raw = self.temp[18]
            eps6_raw = self.temp[19]
            eps7_raw = self.temp[20]
            eps8_raw = self.temp[21]
