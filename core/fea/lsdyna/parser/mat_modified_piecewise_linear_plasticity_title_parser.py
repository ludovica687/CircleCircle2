...
class MatModifiedPiecewiseLinearPlasticityTitleParser(Parser):
    ...
    def _12p0(self, line_raw):
        ...
        if self.line_number == 4:
            ...
            vp_raw = self.temp[13]
            epsthin_raw = self.temp[14]
            epsmaj_raw = self.temp[15]
            numint_raw = self.temp[16]

            eps1_raw = self.temp[17]
            eps2_raw = self.temp[18]
            eps3_raw = self.temp[19]
            eps4_raw = self.temp[20]
            eps5_raw = self.temp[21]
            eps6_raw = self.temp[22]
            eps7_raw = self.temp[23]
            eps8_raw = self.temp[24]
