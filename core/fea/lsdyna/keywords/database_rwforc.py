...
class DatabaseRwforc(ABC):
    """
    user define DatabaseRwforc() object
    input 1 parameters:
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.name == "default":
            self.name = f"database_rwforc_{self.uid}"

    def __repr__(self):
        if self.name == "default":
            return (f"database_rwforc id: {self.uid}, "
                    f"dt: {self.dt}, "
            ...
            return (f"database_rwforc name: {self.name}"
                    f"database_rwforc id: {self.uid}, "
                    f"dt: {self.dt}, "
                    f"binary: {self.binary}, "
                    f"lcur: {self.lcur}, "
                    f"ioopt: {self.ioopt}\n")