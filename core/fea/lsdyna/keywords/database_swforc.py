...
class DatabaseSwforc(ABC):
    """
    user define DatabaseSwforc() object
    input 1 parameters:
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.name == "default":
            self.name = f"database_swforc_{self.uid}"

    def __repr__(self):
        if self.name == "default":
            return (f"database_swforc id: {self.uid}, "
                    f"dt: {self.dt}, "
            ...
            return (f"database_swforc name: {self.name}"
                    f"database_swforc id: {self.uid}, "
                    f"dt: {self.dt}, "
                    f"binary: {self.binary}, "
                    f"lcur: {self.lcur}, "
                    f"ioopt: {self.ioopt}\n")
