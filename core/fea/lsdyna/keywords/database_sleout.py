...
class DatabaseSleout(ABC):
    """
    user define DatabaseSleout() object
    input 1 parameters:
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.name == "default":
            self.name = f"database_sleout_{self.uid}"

    def __repr__(self):
        if self.name == "default":
            return (f"database_sleout id: {self.uid}, "
                    f"dt: {self.dt}, "
            ...
            return (f"database_sleout name: {self.name}"
                    f"database_sleout id: {self.uid}, "
                    f"dt: {self.dt}, "
                    f"binary: {self.binary}, "
                    f"lcur: {self.lcur}, "
                    f"ioopt: {self.ioopt}\n")