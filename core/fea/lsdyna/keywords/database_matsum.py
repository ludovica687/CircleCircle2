...
class DatabaseMatsum(ABC):
    """
    user define DatabaseMatsum() object
    input 1 parameters:
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.name == "default":
            self.name = f"database_matsum_{self.uid}"

    def __repr__(self):
        if self.name == "default":
            return (f"database_matsum id: {self.uid}, "
                    f"dt: {self.dt}, "
            ...
            return (f"database_matsum name: {self.name}"
                    f"database_matsum id: {self.uid}, "
                    f"dt: {self.dt}, "
                    f"binary: {self.binary}, "
                    f"lcur: {self.lcur}, "
                    f"ioopt: {self.ioopt}\n")