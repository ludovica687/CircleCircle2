...
class DatabaseDisbout(ABC):
    """
    user define DatabaseDisbout() object
    input 1 parameters:
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.name == "default":
            self.name = f"database_disbout_{self.uid}"

    def __repr__(self):
        if self.name == "default":
            return (f"database_disbout id: {self.uid}, "
                    f"dt: {self.dt}, "
            ...
            return (f"database_disbout name: {self.name}"
                    f"database_disbout id: {self.uid}, "
                    f"dt: {self.dt}, "
                    f"binary: {self.binary}, "
                    f"lcur: {self.lcur}, "
                    f"ioopt: {self.ioopt}\n")
